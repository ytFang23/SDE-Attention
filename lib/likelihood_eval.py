...
# Import sklearn metrics for F1, accuracy, precision, recall
...
def compute_classification_metrics(label_predictions, true_label, mask=None):
    """
    Compute classification metrics (accuracy, F1, precision, recall).

    This function mirrors the structure of compute_multiclass_CE_loss
    and should be called in the same places.

    Args:
        label_predictions: Model predictions (logits)
            - Sequence-level: [n_traj_samples, n_traj, n_labels] (3D)
            - Per-timepoint: [n_traj_samples, n_traj, n_tp, n_labels] (4D)
        true_label: Ground truth labels (one-hot or indices)
            - Sequence-level: [n_traj, n_labels] (2D)
            - Per-timepoint: [n_traj, n_tp, n_labels] (3D)
        mask: Observation mask (optional, for per-timepoint tasks)
    ...
    """
    # Default metrics
    device = label_predictions.device
    default_metrics = {
    ...
    if len(label_predictions.size()) == 3:
        ...
        # Check true_label dimensions
        if len(true_label.size()) == 2:
            # Sequence-level labels [n_traj, n_labels]
            # Average predictions over MC samples
            label_predictions_avg = label_predictions.mean(dim=0)  # (n_traj, n_labels)

        elif len(true_label.size()) == 3:
            # Dimension mismatch - use last timepoint (for compatibility)
            true_label = true_label[:, -1, :]  # (n_traj, n_labels)
            label_predictions_avg = label_predictions.mean(dim=0)  # (n_traj, n_labels)

        # Convert to numpy
        preds_np = label_predictions_avg.detach().cpu().numpy()
        labels_np = true_label.detach().cpu().numpy()

        # Convert one-hot to class indices
        if labels_np.shape[-1] > 1:
            true_classes = np.argmax(labels_np, axis=-1)
        else:
            true_classes = labels_np.squeeze(-1).astype(int)

        pred_classes = np.argmax(preds_np, axis=-1)

        # Compute metrics
        try:
            acc = float(accuracy_score(true_classes, pred_classes))
            f1 = float(f1_score(true_classes, pred_classes, average='macro', zero_division=0))
            prec = float(precision_score(true_classes, pred_classes, average='macro', zero_division=0))
            rec = float(recall_score(true_classes, pred_classes, average='macro', zero_division=0))

            return {
                'accuracy': torch.tensor(acc, device=device),
                'f1': torch.tensor(f1, device=device),
                'precision': torch.tensor(prec, device=device),
                'recall': torch.tensor(rec, device=device)
            }
    ...
    # ========================================================================
        ...
        # Use LABEL mask: which timepoints have labels
        label_mask = (torch.sum(true_label, dim=-1) > 0).float()  # (n_traj_samples, n_traj, n_tp)

        # Filter to labeled timepoints
        has_labels = label_mask.sum() > 0

        if not has_labels:
            return default_metrics

        # Average over MC samples first
        label_predictions_avg = label_predictions.mean(dim=0)  # (n_traj, n_tp, n_labels)
        true_label_single = true_label[0]  # (n_traj, n_tp, n_labels)
        label_mask_single = label_mask[0]  # (n_traj, n_tp)

        # Flatten and filter
        preds_flat = label_predictions_avg.reshape(-1, n_dims)  # (n_traj*n_tp, n_labels)
        labels_flat = true_label_single.reshape(-1, n_dims)  # (n_traj*n_tp, n_labels)
        mask_flat = label_mask_single.reshape(-1)  # (n_traj*n_tp,)

        # Keep only labeled timepoints
        preds_masked = preds_flat[mask_flat > 0]
        labels_masked = labels_flat[mask_flat > 0]

        # Convert to numpy
        preds_np = preds_masked.detach().cpu().numpy()
        labels_np = labels_masked.detach().cpu().numpy()
...
def compute_binary_classification_metrics(label_predictions, true_label, threshold=0.5):
    """
    Compute binary classification metrics.

    Mirrors compute_binary_CE_loss structure.

    Args:
        label_predictions: Logits [n_traj_samples, n_traj] or [n_traj]
        true_label: Binary labels [n_traj]
    ...
    # Filter NaNs
    idx_not_nan = ~torch.isnan(true_label)
    if idx_not_nan.sum() == 0:
        return default_metrics

    label_predictions = label_predictions[:, idx_not_nan]
    true_label = true_label[idx_not_nan]

    # Average over MC samples
    label_predictions_avg = label_predictions.mean(dim=0)

    # Convert to numpy
    preds_probs = torch.sigmoid(label_predictions_avg).detach().cpu().numpy()
    labels_np = true_label.detach().cpu().numpy()
    ...
    # Compute metrics
    try:
        acc = float(accuracy_score(labels_binary, preds_binary))
        f1 = float(f1_score(labels_binary, preds_binary, zero_division=0))
        prec = float(precision_score(labels_binary, preds_binary, zero_division=0))
        rec = float(recall_score(labels_binary, preds_binary, zero_division=0))
        ...
        print(f"Warning: Binary metric computation failed: {e}")
        return default_metrics
