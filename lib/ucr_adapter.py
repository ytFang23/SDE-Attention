...
def _load_ucr_raw(name: str):
    ...
    # Try to find the dataset directory (case-insensitive fallback for Linux)
    dataset_dir = base_dir / name
    actual_name = name
    if not dataset_dir.exists():
        search_bases = [
            base_dir,
            base_dir / "Univariate2018_ts",
            base_dir / "Univariate_ts",
        ]
        found = False
        for sb in search_bases:
            if not sb.exists():
                continue
            for d in sb.iterdir():
                if d.name.lower() == name.lower() and d.is_dir():
                    dataset_dir = d
                    actual_name = d.name
                    found = True
                    break
            if found:
                break
        if not found:
            possible_dirs = [base_dir / name, base_dir / "Univariate2018_ts" / name,
                             base_dir / "Univariate_ts" / name]
            raise FileNotFoundError(
                f"[UCR] Dataset '{name}' not found. Checked: {possible_dirs}"
            )

    base = dataset_dir / actual_name
