...
def compute_loss_all_batches(model,
    ...
    for i in range(n_batches):
        ...
        # carry attention mixture info from last test batch (non-scalar, not averaged)
        if "attention_mixture_info" in results:
            total["attention_mixture_info"] = results["attention_mixture_info"]

        n_test_batches += 1
        if max_samples_for_eval is not None:
            break

    # average scalars (skip non-scalar keys)
    if n_test_batches > 0:
        for k in list(total.keys()):
            if k == "attention_mixture_info":
                continue
    ...
    if args.classif and classif_predictions.numel() > 0:
        ...
        if "accuracy" not in total and classif_predictions.numel() > 0:
            ...
            if preds.dim() == 3 and labs.dim() == 2:
                ...
                # Compute F1, Precision, Recall using confusion matrix
                # Average predictions across Monte-Carlo samples first
                pred_class_avg = pred_class_id.float().mean(dim=0).round().long()  # [N]

                # Build confusion matrix
                cm = _confusion_matrix(pred_class_avg, true_class_id, C)

                # Compute macro F1, Precision, Recall
                prec, rec, f1 = _precision_recall_f1_macro(cm)
        ...
        for k, v in list(total.items()):
            if k == "attention_mixture_info":  # non-scalar, keep as-is
                continue
