...
class Baseline(nn.Module):
    ...
    def compute_all_losses(self, batch_dict,
        ...
        if (batch_dict["labels"] is not None) and self.use_binary_classif:
            ...
            # No classification - return zeros
        ...
        # Add routing loss for conditional attention mixture (if enabled)
        routing_loss = torch.tensor(0.0, device=device)
        if 'routing_loss' in info:
            routing_loss = info['routing_loss']
        ...
        results["accuracy"] = classification_metrics['accuracy'].detach()
        results["f1"] = classification_metrics['f1'].detach()
        results["precision"] = classification_metrics['precision'].detach()
        results["recall"] = classification_metrics['recall'].detach()
        results["routing_loss"] = routing_loss.detach()  # Add routing loss to results
        ...
        # pass attention mixture info through for routing analysis
        if isinstance(info, dict) and "attention_mixture_info" in info:
            results["attention_mixture_info"] = info["attention_mixture_info"]
