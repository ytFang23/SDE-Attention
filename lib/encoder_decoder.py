...
class Encoder_z0_SDE_RNN(nn.Module):
    def __init__(self, latent_dim, input_dim, z0_diffeq_solver=None,
        ...
        self.conditional_attention_mixture = conditional_attention_mixture

        # Individual attention (used if NOT using mixture)
        self.use_channel_latent_attention = use_channel_latent_attention
        self.channel_latent_attention = channel_latent_attention

        self.use_pyramidal_latent_attention = use_pyramidal_latent_attention
        self.pyramidal_latent_attention = pyramidal_latent_attention

        self.use_tvf_latent_attention = use_tvf_latent_attention
        self.tvf_latent_attention = tvf_latent_attention

        # Store original input data for conditional mixture statistics
    ...
    def run_sdernn(self, data, time_steps,
        ...
        # Extract original data and mask for conditional mixture
        # Assuming data format is [features, mask] concatenated
        half_dim = n_dims // 2
        x_input = data[:, :, :half_dim]
        mask = data[:, :, half_dim:]

        # Store for conditional attention mixture
        self.stored_data = x_input
        ...
        for i in time_points_iter:
            if (prev_t - t_i) < minimum_step:
                ...
                inc = self.z0_diffeq_solver.sde_func.f(prev_t, prev_y) * (t_i - prev_t)
            ...
            if err > 5e-2:
                print(f"[warn] first SDE point != init, |Δ|≈{err:.3e}")

            yi_sde = sde_sol[:, :, -1, :]
            xi = data[:, i, :].unsqueeze(0)

            # === Apply Attention on SDE output before RNN ===

            if self.use_conditional_attention_mixture and self.conditional_attention_mixture is not None:
                # Use Conditional Attention Mixture
                # yi_sde shape: (1, n_traj, latent_dim)
                # Reshape to (n_traj, 1, latent_dim) for attention
                yi_sde_reshaped = yi_sde.permute(1, 0, 2)  # (n_traj, 1, latent_dim)

                # Apply mixture with input statistics
                yi_sde_attn, mixture_info = self.conditional_attention_mixture(
                    yi_sde_reshaped,
                    x_input=self.stored_data,
                    mask=self.stored_mask
                )

                yi_sde = yi_sde_attn.permute(1, 0, 2)  # (1, n_traj, latent_dim)

                # Store mixture info (only once)
                if attention_mixture_info is None:
                    attention_mixture_info = mixture_info

            else:
                # Use individual attention mechanisms

                # Channel Latent Attention
                if self.use_channel_latent_attention and self.channel_latent_attention is not None:
                    yi_sde_reshaped = yi_sde.permute(1, 0, 2)
                    yi_sde_attn, _ = self.channel_latent_attention(yi_sde_reshaped)
                    yi_sde = yi_sde_attn.permute(1, 0, 2)

                # Pyramidal Latent Attention
                if self.use_pyramidal_latent_attention and self.pyramidal_latent_attention is not None:
                    yi_sde_reshaped = yi_sde.permute(1, 0, 2)
                    yi_sde_attn, _ = self.pyramidal_latent_attention(yi_sde_reshaped)
                    yi_sde = yi_sde_attn.permute(1, 0, 2)

                # TVF Latent Attention
                if self.use_tvf_latent_attention and self.tvf_latent_attention is not None:
                    yi_sde_reshaped = yi_sde.permute(1, 0, 2)
                    yi_sde_attn, _ = self.tvf_latent_attention(yi_sde_reshaped)
                    yi_sde = yi_sde_attn.permute(1, 0, 2)
        ...
        # Add attention mixture info to extra_info
        if attention_mixture_info is not None:
            if isinstance(extra_info, list):
                extra_info = {'attention_mixture_info': attention_mixture_info}
            else:
                extra_info['attention_mixture_info'] = attention_mixture_info
...
class Encoder_z0_ODE_RNN(nn.Module):
    ...
    def __init__(self, latent_dim, input_dim, z0_diffeq_solver=None,
        ...
        # Latent Attention mechanisms (on hidden states between ODE and RNN)
        self.use_channel_latent_attention = use_channel_latent_attention
        self.channel_latent_attention = channel_latent_attention
    ...
    def run_odernn(self, data, time_steps,
        ...
        for i in time_points_iter:
            ...
            # ===== Apply Latent-Level Attention (ODE-RNN) =====
            # Channel Latent Attention
            if self.use_channel_latent_attention and self.channel_latent_attention is not None:
                # yi_ode shape: (1, n_traj, latent_dim)
                # Need to reshape to (n_traj, 1, latent_dim) for attention
                yi_ode_reshaped = yi_ode.permute(1, 0, 2)  # (n_traj, 1, latent_dim)
                yi_ode_attn, _ = self.channel_latent_attention(yi_ode_reshaped)
                yi_ode = yi_ode_attn.permute(1, 0, 2)  # (1, n_traj, latent_dim)
            ...
            if self.use_pyramidal_latent_attention and self.pyramidal_latent_attention is not None:
                ...
                # Need to reshape to (n_traj, 1, latent_dim) for attention
                yi_ode_reshaped = yi_ode.permute(1, 0, 2)  # (n_traj, 1, latent_dim)
                yi_ode_attn, _ = self.pyramidal_latent_attention(yi_ode_reshaped)
                yi_ode = yi_ode_attn.permute(1, 0, 2)  # (1, n_traj, latent_dim)
            ...
            if self.use_tvf_latent_attention and self.tvf_latent_attention is not None:
                ...
                # Need to reshape to (n_traj, 1, latent_dim) for attention
                yi_ode_reshaped = yi_ode.permute(1, 0, 2)  # (n_traj, 1, latent_dim)
                yi_ode_attn, _ = self.tvf_latent_attention(yi_ode_reshaped)
                yi_ode = yi_ode_attn.permute(1, 0, 2)  # (1, n_traj, latent_dim)
